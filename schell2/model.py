...
class SegAgent(Agent):
    ...
    def step(self):
        ...
        if self.type == 0:
            if self.similar < 8 * self.model.intolerance0:
                self.model.grid.move_to_empty(self)
            else:
                self.model.happy += 1
                self.model.happy0 += 1
        else:
            if self.similar < 8 * self.model.intolerance1:
                self.model.grid.move_to_empty(self)
            else:
                self.model.happy += 1
                self.model.happy1 += 1
...
class SegModel(Model):
    ...
    def __init__(self, width, height, num_agents, minority_pc, intolerance0, intolerance1):
        self.num_agents = num_agents  # we're allowing these values to be set at each run
        self.minority_pc = minority_pc
        self.intolerance0 = intolerance0
        self.intolerance1 = intolerance1
        self.width = width
        self.height = height
        ...
        self.datacollector = DataCollector(
            model_reporters={"Pct Happy": lambda m: round(100 * m.happy / m.num_agents, 1),
                             ...
                             "Intolerance (A)": lambda m: m.intolerance0,
                             "Intolerance (B)": lambda m: m.intolerance1},
